...
function sys(show, k, g, ep, mode, extras=[], clipCount=5, matchedRules=[], writingStds="") {
...
Write ONLY the sections above. No labels, no commentary, no extra text.`;
  ...
  async function gen(){
    ...
    try{
      ...
      const j=await claudeAPI({model:"claude-sonnet-4-6",max_tokens:mode==="editor"?4000:8000,system:sys(d,show,guest,ep,mode,extraPlatforms,editorClipCount,matched,writingStandards),messages:[{role:"user",content:mode==="editor"?`Analyze this transcript carefully and generate the Editor Brief as instructed.\n\nTRANSCRIPT:\n${tx.substring(0,90000)}`:`Generate the COMPLETE content package in plain text.\n\nTRANSCRIPT:\n${tx.substring(0,90000)}`}]});
