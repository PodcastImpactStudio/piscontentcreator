...
Write ONLY the sections above. No labels, no commentary, no extra text.`;
        ...
        const j=await claudeAPI({model:"claude-3-5-sonnet-20241022",max_tokens:2000,system:clipSys,messages:[{role:"user",content:`CLIP ${i+1} TRANSCRIPT:\n${clipTx.substring(0,8000)}`}]});
  ...
  async function gen(){
    ...
    try{
      const j=await claudeAPI({model:"claude-3-5-sonnet-20241022",max_tokens:mode==="editor"?4000:8000,system:sys(d,show,guest,ep,mode,extraPlatforms,editorClipCount),messages:[{role:"user",content:mode==="editor"?`Analyze this transcript carefully and generate the Editor Brief as instructed.\n\nTRANSCRIPT:\n${tx.substring(0,90000)}`:`Generate the COMPLETE content package in plain text.\n\nTRANSCRIPT:\n${tx.substring(0,90000)}`}]});
  ...
  async function doRev(){
    ...
    try{
      const j=await claudeAPI({model:"claude-3-5-sonnet-20241022",max_tokens:4000,system:revSys(d),messages:[{role:"user",content:`Current:\n\n${raw}\n\n---\n\nRevise "${label}":\n${eTxt}\n\nPlain text only. Only the revised section.`}]});
