...
function normalizeBullets(text){if(!text)return text;const lines=text.split("\n");const out=[];let inTK=false;for(const line of lines){const t=line.trim();if(/^KEY TAKEAWAYS/i.test(t)){inTK=true;out.push(line);continue;}if(inTK){if(!t){out.push(line);continue;}if(t==="---"||(/^[A-Z][A-Z\s&\/():,0-9]+$/.test(t)&&t.length>=3)){inTK=false;out.push(line);continue;}if(!/^[-•]\s/.test(t)){out.push("- "+t);continue;}}out.push(line);}return out.join("\n");}
...
`;
  ...
  // SUBSTACK
  if (email.includes("Substack")) {
    out += `${n++}. SUBSTACK\n[SUBJECT LINE]\n[PREVIEW TEXT]\n\n[Body: 400-700 words. Written as a genuine Substack post — conversational, personal, first-person. Opens with a compelling hook or scene. Weaves in 2-3 key ideas from the episode. Ends with a question or reflection to drive replies and engagement. No corporate tone. Reads like a letter to a reader who cares about the topic.]\n---\n`;
  }
...
[2-3 paragraphs. Personal, warm, conversational tone. As the guest, tell your audience: why you came on this show, who you spoke with (name the host and show), what you discussed, and why your audience should tune in. Draw from specific topics and real moments in the episode — not generic summaries.]

We discuss:
[3-4 bullet points — specific topics covered in the conversation — starting with •]

[Closing invitation: 1 sentence encouraging the reader to listen, ending with:]
🎧 Listen now on Apple Podcasts, Spotify, or YouTube.

[8-10 relevant hashtags on a single line]

LINKEDIN POST:
[Professional opening hook — a bold insight, lesson learned, or honest moment from the episode — written from the guest's POV. Short punchy lines with a line break after each 1-2 sentences.]

[3-4 paragraphs. Professional but human. As the guest: explain why you joined this show, briefly describe your background/business (1-2 sentences), and what prompted you to have this conversation. Name the host and show. Short sentences, frequent line breaks for LinkedIn readability.]

During our conversation we discuss:
[3-4 bullet points — specific topics, insights, or takeaways from the episode — starting with •]

[Closing thought: 1-2 sentences encouraging connections/followers to listen. End with 3-5 professional hashtags.]
🎧 Listen on Apple Podcasts, Spotify, or YouTube.

EMAIL TO LIST:
SUBJECT LINE: [Compelling subject line for the guest's email subscribers — specific and benefit-driven, not generic]
PREVIEW TEXT: [1-sentence preview, 50-90 characters, teasing what's in the email]

[Email body — 3 paragraphs. Guest writing directly to their own email list. Warm and personal opener. Paragraph 1: you recently appeared on [show name] with [host name] and what prompted it. Paragraph 2: 3-4 specific things you talked about in the episode. Paragraph 3: Why this conversation matters to the reader and direct CTA to listen, with [LINK] as a placeholder for the episode URL.]
---
`;
  }
...
function sys(show, k, g, ep, mode, extras=[], clipCount=5, matchedRules=[], writingStds="", platformIntel={}, includeThumbnail=true) {
  ...
  const episodeRuleBlock = matchedRules.length > 0
    ...
    : "";
  const piEntries = Object.entries(platformIntel||{}).filter(([,v])=>v&&v.trim());
  const piBlock = piEntries.length > 0 ? `\n\nPLATFORM GROWTH INTELLIGENCE (updated monthly — incorporate these platform-specific trends when generating content for each platform):\n${piEntries.map(([k,v])=>`\n${k.toUpperCase()}:\n${v.trim()}`).join("\n")}` : "";
  const hookEnabled = !d.snElements?.length || (d.snElements.find(e => e.id === "hook")?.enabled !== false);
  const base = `You are the content strategist for ${d.name}.\n\nPRIMARY RULE\nAlways follow the Show DNA provided for this podcast. It is the source of truth for voice, tone, structure, perspective, and preferred language.\n\nFACTUAL ACCURACY RULE — MANDATORY\nEvery claim, quote, statistic, topic, anecdote, and detail in ALL generated content must be directly sourced from the provided transcript. Do not introduce, invent, or supplement with information from outside the transcript — even if it seems plausible or related to the topic. If a detail cannot be grounded in something actually said or discussed in the transcript, leave it out entirely. Clients review generated content against the actual episode — fabricated or hallucinated content is unacceptable.\n\nVERBATIM QUOTES RULE — NON-NEGOTIABLE\nEvery quote used anywhere in the content package — quote cards, pull quotes, notable quotes, social media posts, show notes, captions, or any other section — must be copied WORD FOR WORD from the transcript. Do not clean up, rephrase, condense, paraphrase, or improve the speaker's words in any way. If the speaker said "um" or stumbled, omit only filler words — but never change the meaning or wording of what was said. The quote must be something the person actually said in this exact episode, not a summary of their point. When in doubt, use the speaker's exact language or do not use a quote at all.\n\nVOICE CONSISTENCY RULE\n- Stay faithful to the podcast's Show DNA. Do not drift into a generic AI, corporate, motivational, or overly polished voice.\n- Write like a real human creator with a point of view.\n- Match the podcast's natural rhythm, attitude, and personality.\n\nANTI-AI LANGUAGE RULE — BANNED VOCABULARY (auto-reject any output containing these)\ndelve · unpack (as a verb for discussing) · dive deep · deep dive (as a noun for the episode) · journey (as a metaphor for change) · unlock · elevate · empower · transformative · game-changer · game-changing · powerful (as a general intensifier) · profound · landscape (metaphorical) · realm · tapestry · testament to · navigate the complexities · at the end of the day · in today\'s fast-paced world · now more than ever · resonate (as filler) · curated · leverage (as a verb) · actionable insights · must-listen · don\'t miss · tune in · you won\'t want to miss · revolutionize · seamlessly · in conclusion · this episode will explore · here are some tips · it\'s important to note · the key is · in this post · here\'s why · in today\'s world\n\nBANNED OPENERS — never open any description, show note, or social post with:\n- "In this episode, we explore / dive into / unpack / break down…"\n- "Join us as…"\n- "Have you ever wondered…"\n- "Whether you\'re [X] or [Y], this episode has something for you"\n- "Ever felt like…" / "Let\'s be honest…" / "Let\'s talk about…" / "Buckle up"\n- Any sentence that names the episode or topic before it names the listener\'s feeling or experience. Open in their world, not in yours.\n\nBANNED CONSTRUCTIONS — never use:\n- "It\'s not just X — it\'s Y" / "This isn\'t about X. It\'s about Y."\n- Abstract tricolons: "clarity, courage, and connection" (three vague nouns strung together)\n- Em dash asides used as a dramatic reveal\n- Two or more rhetorical questions in a row\n- Colon-then-reveal pattern in body copy: "The truth: you were never the problem"\n- Sentence fragments used repeatedly for emphasis. Like this. Every few sentences.\n- Consecutive sentences opening with the same word\n- ALL CAPS for emphasis inside body copy\n- Emoji as bullet points\n- Any sentence that could be dropped into any other show\'s show notes unchanged\n\nAUDIENCE RULE\nWrite to ONE specific person — the show\'s ONE Person from the DNA. Second person only. "You" means that specific person, not a general audience. Never write "listeners," "our audience," "you guys," or "everyone who." Open every description and show note in their experience, not in the topic — the topic can arrive in line two. Specific beats abstract every time: "you re-read the text four times before sending it" beats "you struggle with communication anxiety."\n\nSENSITIVITY RULE\nMany shows on this roster cover stigma-adjacent topics: recovery, mental health, grief, trauma, chronic illness. These rules override CTR considerations and are not optional:\n- Never sensationalise a wound for clicks. The ONE Person may be living the thing named in the title. Write as though they will read it, because they will.\n- No shame framing. "Why you keep failing at sobriety" is out. "Why day one keeps happening" is in. The listener is never the problem — the situation is.\n- No diagnostic or medical claims in any field. The host\'s lived experience is not a clinical statement.\n- No outcome guarantees. Not "how to stop drinking" — at most, what this episode gives them.\n- Keep trigger-adjacent specifics (methods, quantities, graphic detail) out of titles, thumbnails, and show notes headers. They may be in the transcript; they are not promoted into metadata.\n\nSTYLE RULES\n- Natural, direct language. Use contractions. Active voice. Varied sentence structure.\n- Specific, concrete, and grounded. No filler, hype, or corporate jargon.\n- No templates, no summaries. Do not over-explain obvious ideas.\n- Do not repeat the same point in multiple ways.\n- Sentence rhythm: mostly short. Vary length. No more than one sentence in five should run past 25 words.\n\nSHOW DNA USAGE RULE\n- Pull wording, tone, and pacing from the Show DNA first.\n- Use its preferred phrases and naming conventions. Follow its formatting habits.\n- Preserve its level of formality, energy, and humor.\n- If Show DNA conflicts with generic writing best practices, Show DNA wins.\n\nOUTPUT RULE\nReturn only the final content. Do not explain, annotate, or mention these rules.\n\nOUTPUT FORMAT:\n- PLAIN TEXT only. Zero markdown. No asterisks. No bold. No italic.\n- ALL section headers and sub-headers must be in ALL CAPS — every single one, no exceptions\n- This includes: KEY TAKEAWAYS, NOTABLE QUOTE, GUEST BIO, LINKS & RESOURCES, TIMESTAMPS, HASHTAGS, KEYWORDS, SUBJECT LINE, PREVIEW TEXT, and any other label\n- Separate major sections with ---\n- Bullets use - (hyphen space)\n\nCRITICAL RULES:\n1. SEO TITLES: Write the title ONLY. Do NOT add the podcast name, a dash, episode number, or any other text after the title.\n${hookEnabled ? "2. SHOW NOTES: The very first thing after the SHOW NOTES header must be the hook question. No podcast name, no episode info, no intro text." : "2. SHOW NOTES: Do NOT start with a hook question — it is disabled for this show. Follow the show notes template exactly as provided, starting with whatever element appears first in the template."}\n3. BULLETS: KEY TAKEAWAYS must be 3-7 bullet points by default, each on its own line starting with - (hyphen space). Never write takeaways as a paragraph. If the show's AI Rules specify a different bullet count, follow that count exactly.\n4. HEADERS: Never use Title Case for any header or label. ALL CAPS only. "Links & Resources" must be written as "LINKS & RESOURCES".\n5. QUOTES — VERBATIM + ATTRIBUTED: Every quote must be copied EXACTLY as spoken — word for word from the transcript. No paraphrasing, no cleaning up, no improving the wording. EVERY quote must also be attributed by name — format as "exact quote text" — [Name]. Never paraphrase and present it as a direct quote. If you cannot find an exact verbatim quote worth using, do not fabricate one.\n6. PODCAST EPISODE TITLES: Target 45–65 characters (absolute max 80). The primary keyword or topic must appear within the first 40 characters. Formula: [Primary keyword phrase]: [Recognition hook or specific payoff]. No emoji ever. Max 2 ALL-CAPS words. One colon maximum. Guest episodes must name guest AND topic — guest name alone is never a title. Auto-reject: title is only an episode number ("Episode 12"), title is only a guest name ("A Conversation with…").\n7. YOUTUBE TITLES: Target 50–65 characters (platform truncates around 70 — hard ceiling). Primary keyword in the first 3–5 words, inside the first 40 characters. Formula: [Keyword front-loaded] + [curiosity gap or concrete payoff]. No emoji. Max 1–3 ALL-CAPS words. No keyword stuffing or repeated phrases. YouTube title and thumbnail text are a SINGLE UNIT — thumbnail opens a curiosity gap, the title supplies context. They must NOT say the same thing. Maximum 1 shared content word between them.\n8. YOUTUBE DESCRIPTION OPENING: The first 100–150 characters of the YouTube description carry the most algorithmic weight — they are the search snippet and the above-the-fold zone. They must: (a) contain the primary keyword, (b) give a reason to watch, (c) be unique to this video — never open with boilerplate. The first chapter timestamp MUST be 00:00 or YouTube will not build chapters. Minimum 3 chapters. Hashtags: 3–5 only.\n9. SHOW NOTES OPENING: Never open show notes with "In this episode…" or any variant. The first 2 sentences are treated as the search preview. Structure in order: (1) takeaway — what the episode delivers, primary keyword in sentence 1; (2) who it\'s for — one sentence to the ONE Person in their language; (3) what you\'ll learn — 3–5 specific bullets; (4) timestamps; (5) links and resources; (6) one CTA. Boilerplate always at the bottom, never the top.\n\nShow: ${d.name} | "${d.tag}" | Host(s): ${d.hosts}\n${g?"GUEST episode — include Guest Share Kit.":"SOLO episode — skip Guest Share Kit."}${ep?` | Episode ${ep}`:""}\n\nVOICE: ${voice.traits||""} | Energy: ${voice.energy||""} | ${voice.arch||""}\nArc: ${voice.arc||""}\nPhrases: ${(voice.phrases||[]).join(" | ")}\nUSE: ${voice.use||""}\nAVOID: ${voice.avoid||""}\n\nAUDIENCE: ${aud.who||""}\nPain: ${(aud.pains||[]).join(" | ")}\nLanguage: ${aud.lang||""}\n\nPLATFORMS: ${[...ap,...extras].join(", ")} | HASHTAGS: ${d.tags||""}\n${extras.length>0?`ADDITIONAL PLATFORMS THIS EPISODE: ${extras.join(", ")} -- generate a dedicated social post for each additional platform listed.`:""}\n\n${bp ? `BOILERPLATE NOTE: This show has a configured boilerplate. It is appended automatically to the YouTube description and show notes — do NOT include it anywhere in your output.` : ""}\n\nSHOW NOTES FORMAT RULE: The show notes template shown in the section instructions is the COMPLETE format. Follow it EXACTLY — do not add sections, key takeaways, guest bios, or any other content not already specified in the template. Do not add a boilerplate to show notes — it is appended automatically.\n\nTIMESTAMPS RULE: Always include timestamps in the YouTube description. Use the EXACT timestamp markers that appear in the transcript (e.g. if the transcript shows [00:03:47] or 3:47, use 03:47 — NOT a rounded 03:00 or 05:00). Do NOT round to the nearest minute and do NOT invent evenly-spaced times. Each timestamp must mark the precise moment that topic actually begins in the transcript, copied from the nearest transcript marker. If the transcript has no explicit time markers, estimate as closely as possible but still avoid suspiciously round numbers. ${getTimestampsScope(d.snElements) === "both" ? "Also include timestamps in show notes." : "Do NOT include timestamps in show notes unless the show notes template specifically includes them."}${piBlock}\n\nSHOW-SPECIFIC AI RULES — HIGHEST PRIORITY — THESE OVERRIDE ALL DEFAULTS ABOVE:\n${d.rules||"(none)"}${episodeRuleBlock}${writingStds?`\n\nWRITING STANDARDS — PHRASES TO NEVER USE:\nThe following phrases sound AI-generated and must not appear in any published content. When you would naturally use one, rewrite with a specific, concrete detail instead:\n${writingStds}`:""}\n\n`;
...
BEFORE selecting hooks: scan the full transcript for the speaker's foundational phrases, distinctive coinages, and most powerful verbatim language. These are more valuable than any paraphrase.
...
For each hook, identify the ACTUAL STORY — the narrative turning point, admission, reversal, or unexpected moment — not just the surface topic. A hook about "managing stress" is shallow. A hook where someone admits "I had to make my ego take a walk" is a story.
...
QUOTE: [exact verbatim words from the transcript where this moment starts and ends — copy character-for-character — followed by — [Name of who said it]]
WHY THIS WORKS: [the actual story — what happens in this moment, what the speaker admits or reverses, why it's emotionally alive for THIS show's audience specifically]
...
BEFORE selecting clips: scan the full transcript for the speaker's foundational phrases, distinctive coinages, and most powerful verbatim language. These are more valuable than any paraphrase.
...
For each clip, identify the ACTUAL STORY — the narrative turning point, admission, reversal, or unexpected twist — not just the surface topic. A clip about "speech openings" is shallow. A clip about "why a world champion had to humble himself before his coach" is a story.

For each clip:

CLIP #[N]
CLIP TITLE: [4-7 word title that captures the STORY — use the speaker's own words if they're powerful enough. No show name.]
...
QUOTE: [exact verbatim words from the transcript where clip starts and ends — copy character-for-character — followed by — [Name of who said it]]
WHY IT PERFORMS: [the actual story — what happens in this clip, what the speaker admits or reverses, what makes it specific and emotionally alive. Not generic topic descriptions.]
SUGGESTED CAPTION HOOK: [one punchy first line using the speaker's own language when possible]
...
1. SEO TITLE OPTIONS
[5 numbered podcast episode titles. Each must be 45–65 characters, primary keyword within the first 40 characters, formula = [Keyword phrase]: [Recognition hook or specific payoff]. No podcast name, no episode number at the start, no emoji. Do NOT add a RECOMMENDED line.]
---
${sections}`;
}

function revSys(show){const d=show;if(!d)return "";const bp=stripHtml(d.bp||"");return `Content strategist for ${d.name}. PLAIN TEXT only. ALL CAPS headers. - bullets.\nVoice: ${d.voice?.traits||""} | Phrases: ${(d.voice?.phrases||[]).join(" | ")}\nUSE: ${d.voice?.use||""} | AVOID: ${d.voice?.avoid||""}\n${bp?`BOILERPLATE NOTE: This show has a configured boilerplate. It is appended automatically — do NOT include it anywhere in your revision output.\n`:""}Only revise the requested section.`;}
...
function dlDoc(content,filename,bpHtml=""){
  ...
  const out=[];
  let inShowNotes=false,inYoutube=false,bpInserted=false,ytBpInserted=false;
  for(const l of lines){
    const t=l.trim();
    if(t&&TOP_SECTIONS.test(t)){
      // Leaving show notes — inject boilerplate before next section
      if(inShowNotes&&!bpInserted&&bpHtml){out.push(`<div class="bp">${bpHtml}</div>`);bpInserted=true;}
      // Leaving YouTube — inject boilerplate before next section
      if(inYoutube&&!ytBpInserted&&bpHtml){out.push(`<div class="bp">${bpHtml}</div>`);ytBpInserted=true;}
      inShowNotes=/SHOW NOTES/i.test(t);
      inYoutube=/YOUTUBE DESC/i.test(t);
      out.push(`<div class="sec">${t}</div>`);
      continue;
    }
    if(!t){out.push('<p class="blank">&nbsp;</p>');continue;}
    if(t==="---"){out.push('<p class="blank">&nbsp;</p>');continue;}
    if(inYoutube&&!ytBpInserted&&bpHtml&&/^HASHTAGS\b/i.test(t)){out.push(`<div class="bp">${bpHtml}</div>`);ytBpInserted=true;}
  ...
  // Final section was show notes or YouTube — append boilerplate at end
  if(inShowNotes&&!bpInserted&&bpHtml){out.push(`<div class="bp">${bpHtml}</div>`);bpInserted=true;}
  if(inYoutube&&!ytBpInserted&&bpHtml){out.push(`<div class="bp">${bpHtml}</div>`);ytBpInserted=true;}
...
export default function App(){
  ...
  const[writingStandards,setWritingStandards]=useState("");
  const[platformIntel,setPlatformIntel]=useState({});
  ...
  useEffect(()=>{
    async function init(){
      ...
      try{
        ...
        if(data?.value?.writingStandards) setWritingStandards(data.value.writingStandards);
        if(data?.value?.platformIntel) setPlatformIntel(data.value.platformIntel);
      }catch{}
    }
    init();
  },[]);
...
VOICE CONSISTENCY RULE: Do not drift into generic AI, corporate, or motivational language. Write like a real human creator. Match this show's natural rhythm and personality.

ANTI-AI LANGUAGE RULE: Never use: "In today's world", "Let's dive in", "Unlock", "Elevate", "Game-changing", "Seamlessly", "At the end of the day", "In conclusion", "Here are some tips", "The key is", "Here's why".
    ...
    try{
      ...
      const withBp=parsed.map(s=>{
        ...
        if(s.id==="youtube"&&bpRaw){
          ...
          let c=s.content;
          if(/^HASHTAGS\b/im.test(c)){c=c.replace(/^(HASHTAGS\b)/im,`${bpPlain}\n\nHASHTAGS`);}
          else if(/^KEYWORDS\b/im.test(c)){c=c.replace(/^(KEYWORDS\b)/im,`${bpPlain}\n\nKEYWORDS`);}
          else{c=c+`\n\n${bpPlain}`;}
          return{...s,content:c};
        }
        return s;
...
[Repeat for REEL 2, REEL 3, REEL 4]`;
  ...
  async function genEditorSelective(){
    ...
    if(editorSelections.clips)sections.push(`SOCIAL CLIP RECOMMENDATIONS\n\nBEFORE selecting clips: scan the full transcript for the speaker's foundational phrases, distinctive coinages, and most powerful verbatim language. These are more valuable than any paraphrase.\n\nFind exactly ${editorClipCount} moments for high-performing social clips. Each must be between 20 and 45 seconds when spoken — nothing longer.\n\nFor each clip, identify the ACTUAL STORY — the narrative turning point, admission, reversal, or unexpected twist — not just the surface topic. A clip about "speech openings" is shallow. A clip about "why a world champion had to humble himself before his coach" is a story.\n\nFor each:\n\nCLIP #[N]\nCLIP TITLE: [4-7 word title that captures the STORY — use the speaker's own words if they're powerful enough]\nTIMESTAMP: [exact start — exact end]\nDURATION: [estimated — must be 20–45 seconds]\nBEST PLATFORM: [Instagram Reels / TikTok / YouTube Shorts / LinkedIn — pick ONE]\nQUOTE: [exact verbatim words from transcript where clip starts and ends — copy character-for-character — [Speaker]]\nWHY IT PERFORMS: [the actual story — what happens, what the speaker admits or reverses, what makes it specific and emotionally alive]\nSUGGESTED CAPTION HOOK: [one punchy first line using the speaker's own language when possible]`);
